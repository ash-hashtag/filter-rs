...
impl<'a> PageScrollState<'a> {
    pub fn get_lines_in_reverse(&mut self, width: usize, height: usize) {
        ...
        let lines_to_draw = &mut self.current_draw_page;

        lines_to_draw.clear();

        for i in self.page_view.clone().into_iter().rev() {
            let lines = textwrap::wrap(self.page.get_at(i).unwrap(), width);

            for l in lines {
                // lines_to_draw.push(&l);
    ...
    pub fn draw(&mut self, area: ratatui::prelude::Rect, buffer: &mut Buffer) {
        ...
        let lines_in_reverse = &self.current_draw_page;
